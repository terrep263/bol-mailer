...
/**
 * The secret token. Store this in wp-config.php as:
 * define( 'BOL_MCP_TOKEN', 'your-secret-token' );
 *
 * If not defined in wp-config.php, falls back to the option stored in DB.
 */
define( 'BOL_MCP_PROXY_NAMESPACE', 'bol-mcp/v1' );
define( 'BOL_MCP_PROXY_ROUTE',     'mcp' );

/**
 * Register the proxy REST route on init.
 */
add_action( 'rest_api_init', function () {

    // ── GET: SSE handshake (for mcp-remote compatibility) ─────────────────
...
/**
 * Check the token from URL parameter or Authorization header.
 */
function bol_mcp_proxy_check_token( WP_REST_Request $request ): bool|WP_Error {
    ...
    if ( ! empty( $auth_header ) ) {
        $bearer = str_replace( 'Bearer ', '', $auth_header );
        if ( hash_equals( $stored_token, trim( $bearer ) ) ) {
            return true;
        }
    }
...
/**
 * Handle POST — proxy JSON-RPC to the mcp-adapter default server.
 * Authenticates as admin user so all abilities are accessible.
 */
function bol_mcp_proxy_handle( WP_REST_Request $request ): WP_REST_Response {
    // Authenticate as admin for ability permission checks
    ...
    // Forward to mcp-adapter default server REST handler
...
/**
 * Handle GET — return SSE connection confirmation for mcp-remote.
 */
function bol_mcp_proxy_sse( WP_REST_Request $request ) {
    // Return a simple JSON response confirming the endpoint is live
...
function bol_mcp_proxy_settings_page() {
    ...
    $defined_in_config = defined( 'BOL_MCP_TOKEN' );
    $connector_url = rest_url( BOL_MCP_PROXY_NAMESPACE . '/' . BOL_MCP_PROXY_ROUTE ) . '?token=' . urlencode( $token );
    ?>
    <div class="wrap">
        <h1>BOL MCP Proxy</h1>
        <p>Connect Claude.ai to your WordPress site using the URL below. No headers or config files required.</p>

        <table class="form-table">
            <tr>
                <th>Claude.ai Connector URL</th>
                <td>
                    <input type="text" class="regular-text" value="<?php echo esc_attr( $connector_url ); ?>" readonly onclick="this.select()" style="width:100%;font-family:monospace" />
                    <p class="description">Paste this URL into Claude.ai &rarr; Settings &rarr; Connectors &rarr; + Add</p>
                </td>
            </tr>
            <tr>
                <th>Current Token</th>
                <td>
                    <?php if ( $defined_in_config ) : ?>
                        <code><?php echo esc_html( $token ); ?></code>
                        <p class="description">Token is defined in wp-config.php as <code>BOL_MCP_TOKEN</code>. Edit wp-config.php to change it.</p>
                    <?php else : ?>
                        <form method="post">
                            ...
                            <input type="submit" name="bol_mcp_regenerate" class="button" value="Regenerate" onclick="return confirm('Regenerate token? The old connector URL will stop working.')" />
                        </form>
                        <p class="description">Or define permanently in wp-config.php: <code>define( 'BOL_MCP_TOKEN', '<?php echo esc_html( $token ); ?>' );</code></p>
                    <?php endif; ?>
                </td>
            </tr>
        </table>
...
/**
 * Set a default token on activation.
 */
